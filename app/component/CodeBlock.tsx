  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-(--border) bg-[rgb(7,12,20,0.88)] shadow-[0_18px_50px_-35px_var(--accent)]">
      <div className="flex items-center justify-between border-b border-(--border) px-3 py-2 sm:px-4">
        <div className="flex items-center gap-2 text-[0.65rem] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-(--textMuted) overflow-hidden">
        ...
        <button
          type="button"
          onClick={handleCopy}
          className="rounded-lg border border-(--border) px-2 py-1 text-[0.6rem] sm:text-[0.65rem] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-(--textSecondary) transition hover:-translate-y-0.5 hover:text-(--textPrimary) whitespace-nowrap flex-shrink-0"
        >
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
      <pre className="flex-1 overflow-x-auto overflow-y-auto px-3 py-3 sm:px-4 sm:py-4 font-mono text-[0.75rem] sm:text-[0.85rem] leading-relaxed text-(--textSecondary)">
        <code className="block min-w-max" dangerouslySetInnerHTML={{ __html: prettyCode }} />
      </pre>
    </div>
  );
}
